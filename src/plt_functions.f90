...
subroutine read_plt(u,fname,nn,nvar,nc,verbose,dat,n,version)
  ...
  if(verbose.eq.1) write(*,'(A)', advance='no')' Reading ev file '//trim(fname)//':'
  ...
  rewind u
  read(u,*) ncols
  if(verbose.eq.1) write(*,'(A,I4,A)', advance='no')'  Found',ncols,' columns.'
...
subroutine read_bse(u,fname,nn,nvar,verbose,dat,n,version)
  ...
  if(verbose.eq.1) write(*,'(A)', advance='no')' Reading BSE file '//trim(fname)//':'
  ...
  do j=1,nn
     ...
     if(verbose.eq.1.and.j.eq.1) write(*,'(A,F6.2,A)', advance='no')'  Mi =',tmpdat(4),'Mo.'
